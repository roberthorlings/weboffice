            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
			        <table class="table table-bordered table-striped table-hover documents invoices">
			            <thead>
			                <tr>
			                    <th>Date</th><th>#</th><th>Version</th><th>Relation</th><th>Title</th><th>Amount</th><th></th>
			                </tr>
			            </thead>
			            <tbody>
			            @foreach($quotes as $item)
			                ...
			                ">
			                    ...
			                    <td>{{ $item->offertenummer }}</td>
			                    <td>{{ $item->versie }}</td>
			                    <td>{{ $item->Relation ? $item->Relation->bedrijfsnaam : ''}}</td>
			                    <td><a href="{{ url('quote', $item->id) }}" class="title">{{ $item->titel }}</a></td>
			                    <td class="amount">@amount($item->totaalbedrag)</td>
			                    <td align="right">
			                        @if(!$item->definitief)
				                        {!! Form::open([
				                            'method'=>'POST',
				                            'url' => ['quote', $item->id, 'mark_as_final'],
				                            'class' => 'mark_as_final'
				                        ]) !!}
			                        	{!! Form::close() !!}
						            @endif
			                        ...
			                        {!! Form::close() !!}
			                          ...
			                          <div class="btn-group">
							             <a class="btn btn-default btn-xs" href="{{ route('quote.edit', [ 'id' => $item->id ]) }}" title="Edit"><i class="fa fa-pencil fa-fw"></i></a></li>
							             <a class="btn btn-default btn-xs" href="{{ route('quote.pdf', [ 'id' => $item->id ]) }}" title="Download as PDF"><i class="fa fa-download fa-fw"></i></a></li>
                ...
                <div class="box-footer pagination-footer">
			        <div class="pull-right"> {!! $quotes->appends(paginationSafeFilter($filter))->render() !!} </div>
                </div><!-- /.box-footer-->
            </div><!-- /.box -->
        </div><!-- /.col -->
