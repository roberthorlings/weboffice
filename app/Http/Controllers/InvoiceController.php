...
class InvoiceController extends Controller {
	...
	public function index(RelationRepository $repository, Request $request) {
		...
		$query = Invoice::orderBy ( 'factuurnummer', 'desc' )->orderBy ( 'versie', 'desc' );
	...
	public function pdf($id) {
		$invoice = Invoice::findOrFail ( $id );
		$filename = (new Carbon())->format( 'ymd' ) . ' Factuur ' . $invoice->factuurnummer . '.pdf';
		$invoiceNumberPrefix = AppConfig::get ( 'factuurNummerPrefix' );
		return response()
			->view ( 'invoice.pdf', compact ( 'invoice', 'filename', 'invoiceNumberPrefix' ) )
	...
	public function markAsFinal($id) {
		...
		Invoice::where ( 'factuurnummer', $invoice->factuurnummer )->update ( [ 
				'definitief' => false 
		] );
	...
	protected function getDataForProjectForm($invoice, RelationRepository $relationRepository) {
		...
		$preEnteredProjects = array_fill ( 0, $numProjects, [ 
				'id' => null,
				'project_id' => null,
				'start' => $lastMonthStart,
				'end' => $lastMonthEnd,
				'hours_overview_type' => 'defualt' 
		] );
