...
class WorkingHoursController extends Controller {
	...
	protected function getStatistics($filter) {
		$workingHours = WorkingHour::with ( [ 
				'Relation' 
		] )->where ( 'datum', '>=', $filter ['start'] )->where ( 'datum', '<=', $filter ['end'] )->get ();
	...
	public function edit($id, RelationRepository $repository, TravelExpenseRepository $travelExpenseRepository) {
		...
		$relations = $repository->getRelationsWithProjects ( function ($query) {
			return $query->where ( 'type', '<>', Relation::TYPE_SUPPLIER );
		} );
